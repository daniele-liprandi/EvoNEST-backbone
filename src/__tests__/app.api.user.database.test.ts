...
import { getDatabases, setActiveDatabase } from "@/app/api/user/database/route";
...
describe("GET /api/user/database", () => {
  test("returns the user's databases", async () => {
    ...
    const res = await runRoute(
      getDatabases.pipe(Effect.provide(Layer.merge(layer, testAuth({ sub: "u1" })))),
    );
  ...
  test("401 without a session", async () => {
    ...
    const res = await runRoute(getDatabases.pipe(Effect.provide(Layer.merge(layer, testNoAuth))));
    expect(res.status).toBe(401);
  });

  test("404 when the user record is missing", async () => {
    const { layer } = withUser(null);
    const res = await runRoute(getDatabases.pipe(Effect.provide(Layer.merge(layer, testAuth({ sub: "u1" })))));
