...
import { listDatabases, addDatabase } from "@/app/api/databases/handlers";

beforeEach(() => jest.spyOn(console, "error").mockImplementation(() => {}));
afterEach(() => jest.restoreAllMocks());

const admin = testAuth({ sub: "u1", role: "admin" });
const user = testAuth({ sub: "u2", role: "student" });
...
describe("POST /api/databases", () => {
  test("adds a new database when the caller holds databases.manage (admin)", async () => {
    ...
    const res = await runRoute(
      addDatabase(req({ database: "  New_Lab  " })).pipe(Effect.provide(Layer.merge(mongo, admin))),
    );
    expect(res.status).toBe(200);
    await expect(res.json()).resolves.toEqual({ message: "Database added successfully", database: "new_lab" });
  });

  test("403 for a role without databases.manage", async () => {
    // an admin exists, and the default map grants databases.manage to nobody
