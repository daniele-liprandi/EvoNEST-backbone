...
async function up() {
    try {
        ...
        const sourceDatabase = client.db("evonest");
        const backupDatabase = client.db("evonest_backup");

        // Delete the old copy 
        await backupDatabase.dropDatabase();

        // Copy the database
        await copyDatabase(sourceDatabase, backupDatabase);
    } catch (error) {
...
async function down() {
    try {
        await client.connect();
        const sourceDatabase = client.db("evonest_backup");
        const restoredDatabase = client.db("evonest");
