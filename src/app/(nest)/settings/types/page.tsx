    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Add a new item to {configType}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="shortened" className="text-right">
                Shortened
              </Label>
              <Input
                id="shortened"
                ...
                onChange={(e) =>
                  setFormData({ ...formData, shortened: e.target.value })
                }
                className="col-span-3"
              />
            </div>
...
export default function TypesPage() {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (config && config.data) {
              setConfigs((prev: Record<string, LabelType[]>) => ({
                ...prev,
                [configType]: config.data || [],
              }));
            }
          }
        } catch (error) {
  ...
  const setToDefaults = async () => {
    if (
      !confirm(
        "Are you sure you want to set all configurations to defaults? This will overwrite any custom changes."
      )
    ) {
      return;
    }
