...
async function getTraits(client, includeSampleFeatures, type = "") {
    ...
    if (includeSampleFeatures) {
        ...
        traitsData.forEach(trait => {
            const sample = samplesData.find(sample => sample._id.toString() === trait.sampleId.toString());
    ...
    traitsData.forEach(trait => {
        if (trait.type === 'diameter' && trait.measurement) {
            ...
            crossSectionTrait.measurement = Math.PI * Math.pow(trait.measurement/2, 2); // Calculate area
...
export async function POST(req) {
    ...
    if (data.method === "update") {
        const logbookEntry = [`${new Date().toISOString()}`, `Updated trait ${data.id}`];
        const sampleLogbookEntry = [`${new Date().toISOString()}`, `Updated trait ${data.id}`];
        const updateData = {
        ...
        if (result.modifiedCount === 0) {
            return new NextResponse(JSON.stringify({ error: "Failed to update trait" }), { status: 404 });
        } else {
            return new NextResponse(JSON.stringify({ message: "Trait updated successfully" }), { status: 200 });
        }
    }
    ...
    if (data.method === "setfield") {
        ...
        const logbookEntry = [`${new Date().toISOString()}`, `Set ${data.field} to ${data.value} by ${authuser}`];
    ...
    if (data.method === "incrementfield") {
        ...
        const logbookEntry = [`${new Date().toISOString()}`, ` ${data.field} by ${authuser}`];
    ...
    if (data.method === "conversion") {
        ...
        if (!data.traits || !Array.isArray(data.traits) || data.traits.length === 0) {
            return new NextResponse(JSON.stringify({ error: "No traits provided" }), { status: 400 });
        }
        ...
        const firstTrait = await traits.findOne({ _id: new ObjectId(data.traits[0].id) });
        if (!firstTrait) {
            return new NextResponse(JSON.stringify({ error: "Trait not found" }), { status: 404 });
        }

        // Find and update all associated experiments (only once)
        const associatedExperiments = await experiments.find({ sampleId: firstTrait.sampleId }).toArray();

        if (associatedExperiments.length > 0) {
            for (const experiment of associatedExperiments) {
                const rawExperimentData = await rawdata.findOne({ experimentId: experiment._id });
                if (!rawExperimentData) continue;

                const originalData = rawExperimentData.originalData || rawExperimentData.data;
                const updatedData = {
                    ...rawExperimentData.data,
                    EngineeringStress: rawExperimentData.data.EngineeringStress.map(value =>
                        value * data.conversion.ratio
                    ),
                };
                ...
                await experiments.updateOne(
                    { _id: experiment._id },
                    {
                        $set: {
                            ...
                            offsetYieldStress: experiment.offsetYieldStress * data.conversion.ratio,
                            modulus: experiment.modulus * data.conversion.ratio,
                            specimenDiameter: experiment.specimenDiameter / Math.sqrt(data.conversion.ratio) // diameter scales with square root of area
                        },
        ...
        return new NextResponse(
            JSON.stringify({ message: "Traits and experiments updated successfully" }),
            { status: 200 }
        );
    }
    ...
    if (data.method === "reset") {
        ...
        for (const trait of data.traits) {
            ...
            const originalValue = traitEntry.measurement / traitEntry.diameterConversion.ratio;
    ...
    if (data.method === "create") {
        ...
        if (responsibleUser == null) {
            return new NextResponse(JSON.stringify({ error: "Responsible not found" }), { status: 500 });
        }
        ...
        const traitData = {
            ...data,
            recentChangeDate: new Date().toISOString(),
            logbook: [[`${new Date().toISOString()}`, `Uploaded trait for ${data.sampleId} by ${authuser}`]]
        };

        {
            ...
            const sampleResult = await samples.updateOne(
                { _id: new ObjectId(data.sampleId) },
                { $set: { recentTraitChangeDate: new Date().toISOString() }, $push: { logbook: [`${new Date().toISOString()}`, `New trait of type ${data.type} and value ${data.measurement} for ${data.sampleId} by ${authuser}`] } }
            );

            if (result.insertedCount == 0 || sampleResult.modifiedCount == 0) {
                return new NextResponse(JSON.stringify({ error: "Failed to create trait" }), { status: 500 });
            } else {
                return new NextResponse(JSON.stringify({ success: true, id: result.insertedId }), { status: 200 });
            }
        }
    }
...
export async function DELETE(req) {
    try {
        ...
        if (result.deletedCount === 0) {
            return new NextResponse(JSON.stringify({ error: "Trait not found or already deleted" }), { status: 404 });
        }

        return new NextResponse(JSON.stringify({ message: "Trait deleted successfully" }), { status: 200 });
    } catch (error) {
        console.error(error);
